...
class OpenMailApp {
  ...
  static Future<OpenMailAppResult> composeNewEmailInMailApp({
    ...
    if (_isAndroid) {
      ...
      final List<MailApp> installedApps = await _getIosMailApps();
  ...
  ///
  /// [MailApp] (required) the maill app you wish to launchUrl. Get it by calling [getMailApps]
  /// [EmailContent] provides content for the email you're composing
  /// throws an [Exception] if you're launching from an unsupported platform.
  static Future<bool> composeNewEmailInSpecificMailApp({
    ...
    if (_isAndroid) {
      ...
      final String? launchScheme = mailApp.composeLaunchScheme(emailContent);
      if (launchScheme != null) {
...
class ComposeData {
  ...
  String get qsPairSeparator {
    final String separator = !composeStarted ? '?' : '&';
    composeStarted = true;
    return separator;
  }
